...
public record TripPayloadResponse(@NotNull(message = "Não pode deixar este campo sem preencher!") String destination,
    ...
    public TripPayloadResponse(Trip trip) {
        this(trip.getDestination(), trip.getStarts_at(), trip.getEnds_at(), trip.is_confirmed(), trip.getId(), trip.getOwner_name(), trip.getOwner_email(), trip.getParticipantsList());
    }
}
